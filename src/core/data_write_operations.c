...
)
{
    ...
    int err_code=nc_inq_varid(netcdf_file_id, "data", &netcdf_var_id);
    if(err_code != NC_NOERR)
    {
        fprintf(stderr,"Can't find variable \"data\" : %s\n",nc_strerror(err_code));
        return NETCDF_ERROR;
    }
	size_t start[3]={matrix_number_in_file,0,0};
	size_t count[3]={1,size_x,size_y};
    int status = nc_put_vara_double(netcdf_file_id,netcdf_var_id,start,count,data);
	if(status !=NC_NOERR)
	{
        fprintf(stderr, "%s\n", nc_strerror(status));
		return NETCDF_ERROR;
        ...
        )
{
    ...
    int err_code=ncmpi_inq_varid(netcdf_file_id, "data", &netcdf_var_id);
    if(err_code != NC_NOERR)
    {
        fprintf(stderr,"Can't find variable \"data\" : %s\n",ncmpi_strerror(err_code));
        return NETCDF_ERROR;
    }
    MPI_Offset start[3]={matrix_number_in_file,rank,0};
    MPI_Offset count[3]={1,1,size_y};
    ncmpi_begin_indep_data(netcdf_file_id);
    int status = ncmpi_put_vara_double(netcdf_file_id,netcdf_var_id,start,count,data);
    ncmpi_end_indep_data(netcdf_file_id);
    if(status !=NC_NOERR)
    {
