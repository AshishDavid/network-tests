...
int main(int argc,char **argv)
{
    ...
    */
     ...
     *
     */
    flag = easy_mtr_create(&mtr_av,comm_size);
    if( flag==-1 )
    {
    ...
    flag = easy_mtr_create(&mtr_me,comm_size);
    if( flag==-1 )
    {
    ...
    flag = easy_mtr_create(&mtr_di,comm_size);
    if( flag==-1 )
    {
    ...
    flag = easy_mtr_create(&mtr_mi,comm_size);
    if( flag==-1 )
    {
...
//    MPI_Barrier(MPI_COMM_WORLD);
    char *fn[] = {"network_average.nc", "network_min.nc", "network_deviation.nc", "network_median.nc"};
    int *po[] = {&netcdf_file_av, &netcdf_file_mi, &netcdf_file_di, &netcdf_file_me};
    for (int traverse = 0; traverse < 4; traverse++) {
        int err_code = ncmpi_open(MPI_COMM_WORLD, fn[traverse], NC_WRITE | NC_SHARE, MPI_INFO_NULL, po[traverse]);
        if (err_code != NC_NOERR) {
            fprintf(stderr, "Can't open file for write %s: %s\n", fn[traverse], ncmpi_strerror(err_code));
            MPI_Abort(MPI_COMM_WORLD, -1);
            return 1;
        }
    }
            ...
            )
    {
        ...
        if(netcdf_write_matrix_new(netcdf_file_av,netcdf_var_av,step_num,1,mtr_av.sizey,mtr_av.body, comm_rank))
        {
...
//        printf("%d\n  ", step_num);
//        for(int op = 0; op < comm_size; op ++) {
//            printf("%lf ", mtr_av.body[op]);
//        }
//        printf("\n\n");


        if(netcdf_write_matrix_new(netcdf_file_me,netcdf_var_me,step_num,1,mtr_me.sizey,mtr_me.body, comm_rank))
        {
        ...
        if(netcdf_write_matrix_new(netcdf_file_di,netcdf_var_di,step_num,1,mtr_di.sizey,mtr_di.body, comm_rank))
        {
        ...
        if(netcdf_write_matrix_new(netcdf_file_mi,netcdf_var_mi,step_num,1,mtr_mi.sizey,mtr_mi.body, comm_rank))
        {
